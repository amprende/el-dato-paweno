...
function showCustomToast(message, iconName = 'check_circle') {
...
function shareTelegram(title, dealId) {
  const shareUrl = `${window.location.origin}/detalle.html?id=${dealId}`;
  const text = `🔥 ¡Mira esta oferta en Eldatopaweno! 🔥\n\n${title}`;
...
async function shareInstagram(title, dealId) {
  const shareUrl = `${window.location.origin}/detalle.html?id=${dealId}`;
  const shareText = `🔥 ¡Mira esta oferta en Eldatopaweno! 🔥\n${title}`;
  ...
  if (navigator.share) {
    ...
    // Desktop fallback: copy link to clipboard & notify
    navigator.clipboard.writeText(shareUrl).then(() => {
      showCustomToast('¡Enlace copiado! Compártelo en Instagram ✨', 'share');
    }).catch(err => {
