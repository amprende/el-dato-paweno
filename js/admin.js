...
function handlePublishDeal(e) {
  ...
  console.log('Guardado en localStorage. Total custom:', customDeals.length);

  // Actualizar DEALS_DATA en memoria
  DEALS_DATA.unshift(newDeal);
  renderAdminDeals();
...
window.deleteDeal = function(id) {
  ...
  // 1. Quitar de custom_deals (si es de ahí)
  const customDeals = JSON.parse(localStorage.getItem('custom_deals') || '[]');
  ...
  // 2. Si no era custom, es mock. Guardamos el ID en deleted_deals
